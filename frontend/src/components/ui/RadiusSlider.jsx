...
const RadiusSlider = () => {
  ...
  return (
    <div className="absolute bottom-8 left-1/2 -translate-x-1/2 z-[1000] glass-panel p-4 rounded-full flex items-center gap-4 w-11/12 max-w-md">
      <div className="text-xs text-gray-400 uppercase tracking-wider font-bold shrink-0">
        Radius
      </div>
      ...
      <div className="flex-1 flex justify-between items-center bg-black/40 rounded-full p-1 border border-white/5 relative">
        {marks.map((mark) => (
          <button
            ...
            className={`
              relative z-10 px-4 py-2 text-sm font-semibold rounded-full transition-all duration-300
